        ...
        for i in range(0, accountcount):
            ...
            for aid, title, pic ,duration in reports:
                ...
                if "62009" in response.text and skip ==True or reportcount >= limit:
                    print(f'账号{i+1}, 视频{reportcount:03}:{response.text}，{title}')
                    break
                elif "-352" in response.text:
                    print(f'账号{i+1}, 视频{reportcount:03}:{response.text},{title}')
                    capcha(aid, pages[i],i, YOLO_MODEL, YOLO_INPUTS, YOLO_OUTPUTS,
                    ...
                    print(f'账号{i+1}, 视频{reportcount:03}:{response.text}\nhttps://www.bilibili.com/video/av{aid}\n{title}\n')
